 ...
 */
public class AdviserAimDetailActivity extends SwipeBackActivity {
    ...
    private void initView() {
        ...
        adapter = new QuickAdapter<TargetEntity>(this, R.layout.aimdetail_item) {
            ...
            protected void convert(BaseAdapterHelper helper, final TargetEntity item) {
                ...
                ((PaperButton) helper.getView().findViewById(R.id.paper_button)).setText("查看" + FuncUtils.getSubRole(account.getString("roleKey")));
                ...
                if ("医美预收".equals(aim_text.getText().toString())) {
                    ((TextView)helper.getView().findViewById(R.id.receive_rate_text)).setText(FuncUtils.formatMoney2(item.getTarget_medical_beauty())+ "w");
                    ((TextView)helper.getView().findViewById(R.id.fact_rate_text)).setText(FuncUtils.formatMoney2(item.getSale_medical_beauty())+ "w");
                ...
                if ("产品目标".equals(aim_text.getText().toString())) {
                    ((TextView)helper.getView().findViewById(R.id.receive_rate_text)).setText(FuncUtils.formatMoney2(item.getTarget_project())  + "w");
                    ((TextView)helper.getView().findViewById(R.id.fact_rate_text)).setText(FuncUtils.formatMoney2(item.getSale_project()) + "w");
                ...
                if ("消耗目标".equals(aim_text.getText().toString())) {
                    ((TextView)helper.getView().findViewById(R.id.receive_rate_text)).setText(FuncUtils.formatMoney2(item.getTarget_consume()) + "w");
                    ((TextView)helper.getView().findViewById(R.id.fact_rate_text)).setText( FuncUtils.formatMoney2(item.getSale_consume()) + "w");
    ...
    private void initData() {
        ...
        HttpClient.getTargetIndex(entity, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(String body) {
                ...
                if (HttpClient.RET_SUCCESS_CODE.equals(object.getString("status"))) {
                    ...
                    UIHelper.showLogin(AdviserAimDetailActivity.this);
                }else {
                    Toast.makeText(AdviserAimDetailActivity.this, "获取失败",
        ...
        }, AdviserAimDetailActivity.this,true);
    }
    ...
    private void getSubPersonList(String user_id,final String rolekey){
        ...
        if(!StringUtils.isEmpty(param.getUid())){
                    ...
                    if (HttpClient.RET_SUCCESS_CODE.equals(object.getString("status"))) {
                        ...
                        if (list != null && list.size() > 0) {
                            ...
                            if (RoleEnum.CONSULTANT.getCode().equals(rolekey)) {
                                UIHelper.showStaffAimDetail(AdviserAimDetailActivity.this, bundle);
                            } else if (RoleEnum.STOREMANAGER.getCode().equals(rolekey)) {
                                UIHelper.showAdviserAimDetail(AdviserAimDetailActivity.this, bundle);
                            } else if (RoleEnum.AREAMANAGER.getCode().equals(rolekey)) {
                                UIHelper.showStoreAimDetail(AdviserAimDetailActivity.this, bundle);
                            } else if (RoleEnum.BOSS.getCode().equals(rolekey)) {
                                UIHelper.showAreaManagerAimDetail(AdviserAimDetailActivity.this, bundle);
                            }
                        } else {
                            Toast.makeText(AdviserAimDetailActivity.this, "没有其他用户", Toast.LENGTH_LONG).show();
                        }
                    } else {
            ...
            Toast.makeText(AdviserAimDetailActivity.this, "缺少请求参数[uid]", Toast.LENGTH_LONG).show();
        }
